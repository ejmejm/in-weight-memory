...
def gen_train_sequence(
    ...
    """Generate a training sequence for an associative recall task.

    Example sequence: "a=1,b=2,c=3|c=3,a=1,b=2"

    Args:
        rng: random key.
        name_length: number of characters for each variable name. For example, a length of 2 would give names like "ab" or "hq".
        val_length: number of digits for each variable value. For example, a length of 2 would give values like "12" or "49".
        n_vars: number of variables to generate in the sequence.
...
def next_associative_recall_obs(state: ContinualARState) -> Dict[str, Array]:
    """
    Generate the next observation (one token at a time) for the continual associative recall task.
