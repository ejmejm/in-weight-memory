...
class MultiLayerRNNWithAttn(MultiLayerRNN):
    ...
    def forward(
        ...
        for i, layer in enumerate(self.layers):
            ...
            # if memories is not None and i == 0:
            #     # torch.Size([64, 32, 768]) torch.Size([4, 64, 1, 1536])
            #     print(x.shape, memories.shape)
            #     # x = self.integrate_memories(x[:, 0], memories)
...
def parse_args():
    ...
    parser.add_argument('--no_modify_hidden_states', action='store_false', dest='modify_hidden_states', default=True,
                        help='Do not modify hidden states with memories.')
